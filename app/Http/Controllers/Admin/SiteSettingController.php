...
class SiteSettingController extends Controller
{
    ...
    public function homePageStore(Request $request)
    {
        ...
        if (isset(Setting::get('homePage')['chairman']['image'])) {
            $old_image = Setting::get('homePage')['chairman']['image'];
            // $request->merge(['chairman_image' => $old_image]);
            $request->merge([
                'chairman' => array_merge($request->chairman, ['image' => $old_image])
            ]);
        } else {
            $request->merge([
                'chairman' => array_merge($request->chairman, ['image' => ""])
            ]);
        }
        ...
        if ($request->hasFile('file')) {
            ...
            // $request->merge(['chairman_image' => $imageName]);
            $request->merge([
                'chairman' => array_merge($request->chairman, ['image' => $imageName])
            ]);
        }
        ...
        try {
            ...
            if ($request->hasFile('file')) {
                ...
                if (isset($request->cropped_data) && !empty($request->cropped_data && $old_image)) {
                    ...
                    // $request->merge(['chairman_image' => $imageNameUniqid]);
                    $request->merge([
                        'chairman' => array_merge($request->chairman, ['image' => $imageNameUniqid])
                    ]);
                    Setting::update('homePage', $request->except('_token', 'file', 'cropped_data'));
                }
            }
