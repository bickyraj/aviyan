...
var url = document
  .querySelector('script[data-id="banner-list-script"][data-url]')
		...
		dataTable = $('.kt-datatable').KTDatatable({
			...
			columns: [
				...
				},
				{
					...
					template: function(item) {
						return '\
						<a href="'+url+'/admin/banners/edit/'+item.id+'" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Edit details">\
	...
	$(document).on('click', '.kt_sweetalert_delete_page', function(event) {
		...
		swal.fire({
		    ...
		    if (result.value) {
		    	...
		    	var action_url = url + '/admin/banners/delete/' + id;
		    	$.ajax({
		    		url: action_url,
		    		type: "DELETE",
		    		dataType: "json",
